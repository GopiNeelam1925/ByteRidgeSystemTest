...
public class MainActivity extends BaseActivity<ActivityMainBinding, MainViewModel> implements MainNavigator, HasSupportFragmentInjector, OptionClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mActivityMainBinding.setVariable(BR.handler, this);
    ...
    private void setupCardContainerView() {
        ...
        mCardsContainerView.addItemRemoveListener(count -> {
            if (count == 0) {
                ...
                Exam exam = mMainViewModel.getCurrentExam().get();
                if (exam != null) {
                    exam.lookingAt = exam.total_questions - count + 1;
                    mMainViewModel.updateCurrentExamOnSwap(exam);
                }
            }
        });
    }
    ...
    @Override
    public void onOptionClickListener(Button clickFrom, Button button, Option option) {
        Log.e("optionClick", "" + option.isCorrect);
        if (button == clickFrom) {
            Exam localExam = mMainViewModel.getCurrentExam().get();
            if (option.isCorrect) {
                localExam.correct = localExam.correct + 1;
            } else {
                localExam.wrong = localExam.wrong + 1;
            }
            mMainViewModel.updateCurrentExamOnSwap(localExam);
        }
    }
}
