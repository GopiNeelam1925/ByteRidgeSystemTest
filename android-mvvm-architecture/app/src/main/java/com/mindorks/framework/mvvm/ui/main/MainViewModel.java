...
public class MainViewModel extends BaseViewModel<MainNavigator> {
    ...
    private final ObservableField<Exam> exam = new ObservableField<>();
    ...
    public ObservableField<Exam> getCurrentExam() {
        return exam;
    }
        ...
        getCompositeDisposable().add(getDataManager()
                ...
                .subscribe(questionList -> {
                    if (questionList != null) {
                        ...
                        startNewExam(questionList.size());
    ...
    public void startNewExam(int totalQuestions) {
        getCompositeDisposable().add(getDataManager()
                .insertNewExam(new Exam(0, totalQuestions, 0, 0,1))
                .subscribeOn(getSchedulerProvider().io())
                .observeOn(getSchedulerProvider().ui())
                .subscribe(this::getCurrentExamFromDb, throwable -> {
                    Log.d(TAG, "startNewExam: " + throwable);
                }));
    }

    private void getCurrentExamFromDb(long examId) {
        getCompositeDisposable().add(getDataManager()
                .getCurrentExam(examId)
                .subscribeOn(getSchedulerProvider().io())
                .observeOn(getSchedulerProvider().ui())
                .subscribe(this.exam::set, throwable -> {
                    Log.d(TAG, "getCurrentExamFromDb: " + throwable);
                }));
    }

    public void updateCurrentExamOnSwap(Exam exam) {
        getCompositeDisposable().add(getDataManager()
                .updateCurrentExam(exam)
                .subscribeOn(getSchedulerProvider().io())
                .observeOn(getSchedulerProvider().ui())
                .subscribe(examId->{
                    Log.d(TAG, "updatedExam: " + examId);
                  //  getCurrentExamFromDb(exam.examId);
                }, throwable -> {
                    Log.d(TAG, "updatedExam: " + throwable);
                }));
    }
}
