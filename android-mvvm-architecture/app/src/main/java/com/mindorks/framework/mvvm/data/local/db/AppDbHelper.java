...
public class AppDbHelper implements DbHelper {
    ...
    @Override
    public Observable<Long> insertNewExam(Exam exam) {
        return Observable.fromCallable(new Callable<Long>() {
            @Override
            public Long call() throws Exception {
                return mAppDatabase.examDao().insertNewExam(exam);
            }
        });
    }

    @Override
    public Flowable<Exam> getCurrentExam(long examiId) {
        return mAppDatabase.examDao().getCurrentExam(examiId);
    }

    @Override
    public Observable<Integer> updateCurrentExam(Exam exam) {
       return Observable.fromCallable(new Callable<Integer>() {
            @Override
            public Integer call() throws Exception {
                return mAppDatabase.examDao().updateCurrentExam(exam);
            }
        });
    }
}
